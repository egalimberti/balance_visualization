...
export class ViewGraphGeneratorService {
     ...
     */
    public callServiceGenerateGraph(request: GraphRequest): Observable<GraphResponse> {
        return this.http.post('/Controller', {
            dtype: request.dType,
            graph: request.graph
        }) as Observable<GraphResponse>;
    }
     ...
     * @param request request for graph generation
     */
    public processGraphRequest(request: GraphRequest, decimalPrecision = 10): GraphResponse {
    ...
    private buildLaplacianMatrix(nodes: Array<Node>, edges: Array<Edge>): Array<Array<number>> {
        const result = new Array<Array<number>>(nodes.length).fill(undefined).map(() => new Array<number>(nodes.length).fill(0));
