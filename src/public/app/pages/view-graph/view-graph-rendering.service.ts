...
export class ViewGraphRenderingService {
     ...
     */
    public async showGraph(data: GraphResponse, config: GraphRenderingConfig): Promise<void> {
    ...
    private async renderer(data: GraphResponse, config: GraphRenderingConfig): Promise<void> {
    ...
    private enableZoom(svg: D3SvgElement, renderingData: GraphRenderingData, nodeSvg: NodeSelection, interEdgeSvg: EdgeSelection, intraEdgeSvg: EdgeSelection, overlapRemovalMode: OVERLAP_REMOVAL): void {
        const getXAxisTranslatePos = (scaleFunction: Function) => {
            const newPos = scaleFunction(renderingData.dTypeCoefficient !== 0 || renderingData.eigenValue === 0 ? 0 :
        ...
        const getYAxisTranslatePos = ((scaleFunction: (n: number) => number) => {
        ...
        function updateChart() {
            ...
            const newX = d3.event.transform.rescaleX(zoomScaleX);
            const newY = d3.event.transform.rescaleY(zoomScaleY);
     ...
     * @param showUnbalancement whether to show graph unbalancement
     */
    private highlight(
    ...
    private printAxes(svg: D3SvgElement, renderingData: GraphRenderingData): void {
        ...
        if (renderingData.eigenValue === 0) {
            ...
            const xAxis = svg.append('line')
                ...
                .attr('stroke', HEX_COLORS.BLACK);
            // x-axis origin
            const xAxisOrigin = svg.append('line')
                .attr('x1', renderingData.scaleLinearX(-1))
