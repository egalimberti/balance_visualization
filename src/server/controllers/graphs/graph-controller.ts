...
import { DTYPE } from '../../consts';
import { Edge, Node, SignType, SpectralAnalysis, GraphEdges, Eig } from '../../models';
import { readdirSync, readFileSync } from 'fs-extra';

export class GraphController extends CrudController {
    public create(req: Request<import('express-serve-static-core').ParamsDictionary, GraphResponse, GraphRequest>, res: Response<GraphResponse | string>): void {
        if (!!req.body && !!(req.body as GraphRequest).dType && !!(req.body as GraphRequest).graph) {
            if (req.query.hasOwnProperty('dp') && Number.parseInt(req.query.dp.toString(), 10)) {
                res.status(201).json(this.processGraphRequest(req.body as GraphRequest, Number.parseInt(req.query.dp.toString(), 10)));
            } else {
                res.status(201).json(this.processGraphRequest(req.body as GraphRequest));
            }
        } else {
            res.status(400).send();
        }
    ...
    public read(req: Request<import('express-serve-static-core').ParamsDictionary>, res: Response): void {
        if (req.query.hasOwnProperty('entry')) {
            try {
                const reqFile = readFileSync(`./data/${req.query.entry}.dat`, 'utf8');
            ...
            try {
                const allEntries = Array.prototype.concat.apply([],
                    readdirSync('./data')
                        .filter((entry) => entry.indexOf('.') < 0 !== entry.indexOf('.dat') >= 0)
                        .map((entry: string) => entry.indexOf('.dat') >= 0 ? entry :
                            readdirSync(`./data/${entry}`).map((subentry: string) => `${entry}/${subentry}`)))
                    .filter((finalentry: string) => finalentry.indexOf('.dat') >= 0)
                    .map((finalentry: string) => finalentry.split('.')[0])
                res.status(200).json(allEntries);
            } catch (error) {
                res.status(404).send('No files to show - ' + error);
            }
        }
    }
     ...
     */
    public processGraphRequest(request: GraphRequest, decimalPrecision = 8): GraphResponse {
        ...
        for (const nodeKey in nodesMap) {
            if (nodesMap.hasOwnProperty(nodeKey) && progressiveIndex !== nodesMap[nodeKey].id) {
        ...
        spectral.dtypeCoefficient = this.calcDtypeCoefficient(spectral.nodes, dtype);

        // generating response
        const definitiveEdges: GraphEdges = this.getDefinitiveEdges(spectral.edges, spectral.nodes);
    ...
    public getDefinitiveEdges(edges: Array<Edge>, nodes: Array<Node>): GraphEdges {
        const intraEdges: Array<Edge> = new Array<Edge>();
        const interEdges: Array<Edge> = new Array<Edge>();

        const generateDefinitiveEdge = (e: Edge, sourceNode: Node, targetNode: Node): Edge => {
            const result: Edge = {
                ...e,
                consecutive: (sourceNode.cluster === targetNode.cluster) && (Math.abs(sourceNode.originalY - targetNode.originalY) === 1)
            };
            if ((sourceNode.x > targetNode.x) || (sourceNode.x === targetNode.x && sourceNode.y < targetNode.y)) {
    ...
    private buildLaplacianMatrix(nodes: Array<Node>, edges: Array<Edge>): Array<Array<number>> {
        const result = new Array<Array<number>>(nodes.length).fill(new Array<number>()).map(() => new Array<number>(nodes.length).fill(0));
    ...
    private setNodesClusters(nodes: Array<Node>, eigVector: Array<number>, xScaleFactor: number, yScaleFactor: number): Array<Node> {
    ...
    private eigenDecomposition(L: Array<Array<number>>, decimalPrecision: number): Eig {
        ...
        const getMinOrMaxValueIndex = (values: Array<number>, getMax: boolean): number => {
            let index = 0;
            let value = values[0];
        ...
        */
        ...
        const eigenVector: Array<number> = svdEigenVectors.map((eigVector: Array<number>) => eigVector[minEigenValueIndex]);
        ...
        for (let i = 0; i < eigenVector.length; i++) {
            const roundValue = (Math.round((eigenVector[i] / maxAbsValue) * Math.pow(10, decimalPrecision)) / Math.pow(10, decimalPrecision));
