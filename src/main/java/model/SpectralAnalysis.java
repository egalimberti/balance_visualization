...
public class SpectralAnalysis {
    ...
    private double[] getEigenvector() {
        ...
        EigenDecomposition eigenDecomposition = new EigenDecomposition(matrix, MathUtils.SAFE_MIN);
        ...
        RealVector realEigenvector = eigenDecomposition.getEigenvector(index);
        double[] eigenvector = new double[realEigenvector.getDimension()];
        for (int i = 0; i < eigenvector.length; i++) {
            eigenvector[i] = realEigenvector.getEntry(i);
        }
        ...
        for (int i = 0; i < eigenvector.length; i++) {
            eigenvector[i] = realEigenvector.getEntry(i) / maxAbsValue;
        }
